...
export class MergeState {
  ...
  public static renderInDomElement(unique_id: string, merge_input: MergeInput) {
    ...
    for (const k in merge_input) {
      ...
      if (error != null) {
        templates.push(html`
          <div id="details_${k_uid(k)}" class="merge-view collapsed">
            <div class="merge-view-header">
              <code>${k}</code><span class="if-collapsed">: ${error}</span>
            </div>
        ...
        templates.push(html`
          <div id="details_${k_uid(k)}" class="merge-view expanded">
            ...
            <div class="merge-view-content" id="cm_${k_uid(k)}"></div>
          </div>
        `);
      }
    }
  ...
  protected createCodeMirrorMergeWidget(
    ...
    const detailsEl = <HTMLDivElement>(
      document.getElementById(`details_${unique_id}`)!
    );
    const headerEl = <HTMLDivElement>(
      detailsEl.querySelector(".merge-view-header")!
    );
    ...
    // Starting with the view collapsed breaks CodeMirror, especially line numbers
    // in left and right merge view. So we start expanded and then collapse.
    const isExpanded = () => detailsEl.classList.contains("expanded");
    const toggleCollapsed = () => {
      if (this.singleEditor()) {
        // Don't allow collapsing in single editor mode
    ...
    // Start collapsed (after CodeMirror is initialized)
    setTimeout(() => {
      if (!this.singleEditor()) {
        detailsEl.classList.remove("expanded");
        detailsEl.classList.add("collapsed");
  ...
  protected recreateCodeMirrorFlippingOption(
    ...
    // TODO: Perhaps one day we'll want to toggle this option when the view is collapsed.
    detailsEl.classList.remove("collapsed");
    detailsEl.classList.add("expanded");
    ...
    for (const merge_view of this.parent_window.getElementsByClassName(
      ...
      if (merge_view.id == `details_${unique_id}`) {
        ...
        if (merge_view.classList.contains("pinned-mode-selected")) {
          // Ensure the view is expanded when pinned
